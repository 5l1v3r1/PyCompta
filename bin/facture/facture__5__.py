...
def logo_pyfacture():
	...
	print("╔────────── ╩   ┴ ╚  ┴ ┴└─┘ ┴ └─┘┴└─└─┘ ────────────────────────────╗")
	time.sleep(0.08)
	print("| Ce Medule Calcule La facture Avec Touts Cest composant, le Rabais |")
	time.sleep(0.08)
	print("| Remis, Ristourn, L'escompt, le Trasport, l'embalage, est La TVA   |")
	time.sleep(0.08)
	print("| General.                                                          |")
	time.sleep(0.08)
	print("╚───────────────────────────────────────────────────────────────────╝")
				...
				try:
					if not user_input_rabais_S:
						...
						try:
							...
							print("['_'] Resultat Net Commercial après le Rabais et", "{0:.2f}".format(net_comRabais1))
									...
									try:
										if not rab2_S:
											...
											try:
												...
												print("['_'] Resultat Net Commercial après le 2em Rabais et", "{0:.2f}".format(net_comRabais2))
												rab2_loop = False
												#--------------------------------
														...
														try:
															if not rab3_S:
																...
																try:
																	...
																	print("['_'] Resultat Net Commercial après le 3em Rabais et", "{0:.2f}".format(net_comRabais3))
																	rab3_loop = False
																	rab2_loop = False
																	#---------
				...
				try:
					if not user_input_rem_S:
						...
						try:
							...
							print("['_'] Resultat Net Commercial après La Remise et", "{0:.2f}".format(net_comRemis1))
							rem1_loop = False
						#------------------------------------------------------
									...
									try:
										if not rem2_S:
											...
											try:
												...
												print("['_'] Resultat Net Commercial après La 2em Remise et", "{0:.2f}".format(net_comRemis2))
												rem2_loop = False
											#------------------------------------------------------
														...
														try:
															if not rem3_S:
																...
																try:
																	...
																	# remis calculation and declaration
																	global net_comRemis3
																	global rem3_val
																	rem3_val = net_comRemis2 * rem3_float / 100
																	net_comRemis3 = net_comRemis2 - rem3_val

																	# print the result
																	print("['_'] Resultat Net Commercial après La 3em Remise et", "{0:.2f}".format(net_comRemis3))
																	rem3_loop = False
																#------------------------------------------------------
				...
				try:
					if not rist1_S:
						...
						try:
							...
							print("['_'] Resultat Net Commercial après Ristourn et", "{0:.2f}".format(net_comRistourn1))
									...
									try:
										if not rist2_S:
											...
											try:
												...
												print("[*] Net Comercial 'NC':", "{0:.2f}".format(net_comRistourn2))
												rist2_loop = False
											#------------
				...
				try:
					if not escom1_S:
						...
						try:
							...
							print("['_'] Resultat Net Financier après L'escompt et", "{0:.2f}".format(netF_Escomp1))
					...
					while transport_forfitaire_loop:
						...
						try:
							if not transport_value:
								...
								# prit the results
								print("['_'] Resultat du TVA-Forfitaire : ", "{0:.2f}".format(tva_forfitaire_result))
								print("['_'] Resultat du Transport + TVA-Forfitaire :", "{0:.2f}".format(trans_forfitaire_ttc_result))
								# exit the loops
					...
					while transport_debours_loop:
						...
						try:
							if not transport_value:
								...
								# prit the results
								print("['_'] Resultat du TVA-Debours : ", "{0:.2f}".format(tva_debours_result))
								print("['_'] Resultat du Transport + TVA-Debours :", "{0:.2f}".format(trans_debours_ttc_result))
								# exit the loops
								...
								try:
									...
									while tva_port_loop:
										...
										try:
											if not tva_val_str:
												...
												try:
													...
													print("['_'] Resultat du TVA-PORT : ", "{0:.2f}".format(tva_port_result))
										...
										# handle Error from the second loop
											...
											exit()
								#----------
									print("['_'] Resultat du Transoirt + TVA sur Tansport :", "{0:.2f}".format(port_ttc_result))
									manual_transport_loop = False
				...
				try:
					if not emb_unit_str:
						...
						try:
							...
							#-------
										print("['_'] Resultat d'Embalage : ", "{0:.2f}".format(emb_value))
		...
		while taux_tva_loop:
			try:
				...
				if not taux_tva_string:
					...
					print("['_'] Resultat TVA-General :", "{0:.2f}".format(tva_general))
					# Exit the loop
					taux_tva_loop = False
...
def get_net_ttc():
	...
	print("['_'] Resultat Net TTC :", "{0:.2f}".format(net_ttc))
...
def print_all():
	...
	print(" [-_-] La Facture")
	print("┌───────────────────────────────────")
	print("| MB                    ", "{0:.2f}".format(hTax_F))
	print("| Rabais  ", rab_float, 	"%	", "{0:.2f}".format(rab1_val))
	print("| Rabais2 ", rab2_float,	"%	", "{0:.2f}".format(rab2_val))
	print("| ----------------------")
	print("| NetC                  ", "{0:.2f}".format(last_netCom_rab_result))
	print("| Remise1 ", rem1_float,	"%	", "{0:.2f}".format(rem1_val))
	print("| Remise2 ", rem2_float,	"%	", "{0:.2f}".format(rem2_val))
	print("| ----------------------")
	print("| NetC                  ", "{0:.2f}".format(last_NC_remis_result))
	print("| Ristourn", rist1_float,"%	", "{0:.2f}".format(net_comRistourn1))
	print("| ----------------------")
	print("| NetC                  ", "{0:.2f}".format(last_NC_rist_result))
	print("| Escompt ", escom1_float ,"%	", "{0:.2f}".format(escom1_val))
	print("| ----------------------")
	print("| NetF                  ", "{0:.2f}".format(last_NF_result))
	
	if tva_forfitaire_result > 0 and trans_forfitaire_ttc_result > 0:
		print("| Transport         	", "{0:.2f}".format(transport_forfitaire_value_float))
		print("| Tva Port 20 %		", "{0:.2f}".format(last_tva_transport_result))
		
	elif tva_debours_result > 0 and trans_debours_ttc_result > 0:
		print("| Transport         	", "{0:.2f}".format(transport_debours_value_float))
		print("| Tva Port 14 %		", 	"{0:.2f}".format(last_tva_transport_result))
		
	else:
		print("| Transport         	", "{0:.2f}".format(port_val_float))
		print("| Tva Port", tva_val_float, "%	", "{0:.2f}".format(last_tva_transport_result))
	
	print("| Embalage          	", "{0:.2f}".format(emb_value))
	print("| TVA     ", taux_tva_float, 	"%	", "{0:.2f}".format(tva_general))
	print("| ----------------------")
	print("| Net TTC           	", "{0:.2f}".format(net_ttc))
	print("└───────────────────────────────────")

def hold_befor_quit():
	print("")
	user_enter_click = input("Appuyer sur la Touch Entre pour Continue. ")
	exit()
